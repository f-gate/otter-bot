...
use teloxide::types::{Recipient, InlineKeyboardMarkup, InlineKeyboardButton, CountryCode, InlineQueryResultArticle, InputMessageContent, InputMessageContentText, Message, InlineQuery, ChatId, ChatMember, UserId};
...
impl Questions {
        ...
        pub async fn start(bot: Bot, dialogue: MyDialogue, msg: Message) -> HandlerResult {
            ...
            let bot_is_admin = admins.iter().any(|a|a.user.is_bot && a.user.username.as_ref().is_some_and(|n| n == bot_name));
            if !ChatId::is_group(msg.chat.id) {
                bot.send_message(msg.chat.id, "Please add me to a group chat to function correctly.").await?;
            }
            if  !bot_is_admin {

...
pub enum State {
    ...
    Location {full_name: String, referee: UserId},
    TypeOfPersonList {full_name: String, referee: UserId, location: String},
    WhatAreYouBuilding {full_name: String, referee: UserId, location: String, person: PersonType},
}
...
async fn ask_full_name(bot: Bot, dialogue: MyDialogue, msg: Message) -> HandlerResult {
    match msg.text() {
        Some(text) => {
            ...
            bot.send_message(msg.chat.id, format!("And with whom are you chatting with? (@alexanderfive, @xxx, @yyy, or @zzz) ")).await?;
...
async fn ask_who_with(bot: Bot, dialogue: MyDialogue, msg: Message) -> HandlerResult {
    match msg.text() {
        Some(text) => {
